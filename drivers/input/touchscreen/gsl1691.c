/*
 * drivers/input/touchscreen/gsl1691/gsl1691.c
 *
...
#define GSLX680_I2C_NAME	"gsl1691"
...
static const struct key_data gsl_key_data[MAX_KEY_NUM] = {    //实际触摸部分会更大，暂定720*1600，显示部分720*1440，因为屏幕底部还有两个虚拟按键，按键位置参数待定，地球仪是主页，橡皮擦是返回
	{KEY_BACK, 430, 430, 620, 620},                  //右下角位置
	{KEY_HOMEPAGE, 20, 20, 620, 620},               //左下角位置
...
static int gslX680_init(struct gsl_ts *ts)
{
	...
	ts->irq = of_get_named_gpio_flags(np, "irq_gpio", 0, NULL);
	ts->rst = of_get_named_gpio_flags(np, "rst_gpio", 0, NULL);
	...
	if (err) {
		pr_err("gsl1691 reset gpio request failed.\n");
		return -1;
	}
...
{
	pr_info("gsl  gsl1691_shutdown_low\n");
...
{
	pr_info("gsl  gsl1691_shutdown_high\n");
	gpio_direction_output(ts->rst, 1);
	gpio_set_value(ts->rst, 1);
...
static void init_chip(struct i2c_client *client, struct gsl_ts *ts)
{
	...
	rc = test_i2c(client);
	if (rc < 0) {
		dev_err(&client->dev, "gsl1691 test_i2c error\n");
		return;
	}
	clr_reg(client);
	reset_chip(client);
...
static u16 map_coordinate(u16 raw_val, u16 raw_min, u16 raw_max, u16 target_max)
{
    if (raw_max <= raw_min || target_max == 0)
        return 0;
    
    /* 映射原始坐标到目标坐标 */
    if (raw_val <= raw_min)
        return 0;
    else if (raw_val >= raw_max)
        return target_max;
    else
        return ((raw_val - raw_min) * target_max) / (raw_max - raw_min);
}

static void report_data(struct gsl_ts *ts, u16 x, u16 y, u8 pressure, u8 id)
{
    u16 mapped_x, mapped_y;
    /* 
	Hack:
	根据getevent数据分析，原始坐标范围是X:160-704, Y:483-1424
	映射到目标屏幕范围480x640 
	*/
    mapped_x = map_coordinate(x, 160, 704, SCREEN_MAX_X);
    mapped_y = map_coordinate(y, 483, 1424, SCREEN_MAX_Y);
    
    /* 确保坐标在有效范围内 */ 
    if (mapped_x > SCREEN_MAX_X) mapped_x = SCREEN_MAX_X;
    if (mapped_y > SCREEN_MAX_Y) mapped_y = SCREEN_MAX_Y;
	...
	delt_x = (int)mapped_x - old_x;
	delt_y = (int)mapped_y - old_y;
	delt_x /= 10;
	delt_y /= 10;
	...
	//x = 720 - x;               //X轴对调
	//y = 1440 - y;               //实际触摸部分会更大，暂定1600，显示部分720*1440，因为屏幕底部还有两个虚拟按键，按键位置参数待定，地球仪是主页，橡皮擦是返回
	//swap(x, y);                //x和y轴坐标对换

	input_mt_slot(ts->input, id);
	input_report_abs(ts->input, ABS_MT_TRACKING_ID, id);
	input_report_abs(ts->input, ABS_MT_TOUCH_MAJOR, pressure);
	input_report_abs(ts->input, ABS_MT_POSITION_X, mapped_x);
	input_report_abs(ts->input, ABS_MT_POSITION_Y, mapped_y);
...
//参考链接https://blog.csdn.net/laohuang1122/article/details/12209383
...
MODULE_DEVICE_TABLE(of, gsl_ts_ids);
...
MODULE_AUTHOR("Shanghai Basewin");
MODULE_DESCRIPTION("Silead GSL1691 touchscreen driver");
